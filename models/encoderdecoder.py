...
from interfaces import Generator, TrainingMonitor
...
class Base(Generator, ABC):
    ...
    def fit(self, train_loader, valid_loader, epochs=100, patience=50, evaluator=None, monitor=None, no_multifrag_smiles=True):
        ...
        for epoch in tqdm(range(epochs), desc='Fitting model'):
            ...
            print(f"Epoch: {epoch} Validation loss: {loss_valid:.3f} Valid: {valid:.3f} FragsDesire: {frags_desire:.3f} Time: {int(t1-t0)}s")
...
class Seq2Seq(SmilesFragsGeneratorBase):
    def __init__(self, voc_src, voc_trg, emb_sharing=True, device=utils.dev):
...
class EncDec(SmilesFragsGeneratorBase):
    def __init__(self, voc_src, voc_trg, emb_sharing=True, device=utils.dev):
        super(EncDec, self).__init__(device=device )
        self.mol_type = 'smiles'
        self.voc_size = 128
        self.hidden_size = 512
        self.voc_src = voc_src
